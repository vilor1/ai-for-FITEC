...
function construirIA(treinamento){
    ...
    const palavrasChave = [...new Set(
        treinamento.flatMap(t => tokenizar(t.frase))
    )]

    const vetorizar = frase =>
        palavrasChave.map(p =>
            tokenizar(frase).includes(p) ? 1 : 0
        )
    ...
    treinamento.forEach(exemplo => {
        ...
        if(!pesos[exemplo.categoria]){
            pesos[exemplo.categoria] = new Array(palavrasChave.length).fill(0)
        }

        vetor.forEach((bit,i)=>{
            if(bit === 1) pesos[exemplo.categoria][i]++
        })
...
function prever(treinamento, frase){
    ...
    if(treinamento.length === 0){
        return {intencao:"Sem treino",confianca:"0%"}
    }
    ...
    Object.keys(ia.pesos).forEach(cat=>{
        ...
        const score =
            vetorEntrada.reduce((acc,bit,i)=>
                acc + (bit * ia.pesos[cat][i])
            ,0)
