...
// ====== PESQUISA WEB (API STABLE) ======
async function pesquisarWeb(query) {
    return new Promise(resolve => {
        // Usando uma instância do SearXNG que retorna JSON para evitar bloqueios de HTML
        const options = {
            hostname: 'searx.be', 
            path: `/search?q=${encodeURIComponent(query)}&format=json`,
            method: 'GET',
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AI-Assistant/1.0'
            }
        };
            ...
            res.on("end", () => {
                try {
                    ...
                    if (json.results && json.results.length > 0) {
                        // Pega o conteúdo do primeiro resultado e limpa tags se houver
                        let resposta = json.results[0].content.replace(/<[^>]+>/g, "");
                        resolve(resposta.slice(0, 500));
                    } else {
                        resolve("Não encontrei resultados específicos na internet.");
                    }
                } catch (e) {
                    resolve("Busca concluída, mas o formato de resposta foi inválido.");
                }
            });
        });

        req.on("error", () => resolve("Serviço de busca temporariamente indisponível."));
        req.setTimeout(7000, () => { req.destroy(); resolve("A pesquisa excedeu o tempo limite."); });
    });
}
...
async function obterResposta(modelo, frase, pred) {
    ...
    if (parseFloat(pred.confianca) > 70 && modelo.respostas && modelo.respostas[pred.intencao]) {
        return modelo.respostas[pred.intencao]["__default"];
    }
